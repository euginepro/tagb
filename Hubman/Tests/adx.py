...
from ep_movies.earnspro_links import EarnsProLinkManager
    ...
    if ad_random <= ctr:
        ...
        try:
            element_to_click = WebDriverWait(m_browser, 10).until(
                EC.presence_of_element_located((By.ID, "google_ads_iframe_/23158947121/earnsEpAd_0"))
            )
...
def visit_site_direct(d_browser, page_views):
    ...
    try:
        ...
        d_browser.get(EarnsProLinkManager().get_link())
        print("waiting 5s")
        time.sleep(5)
